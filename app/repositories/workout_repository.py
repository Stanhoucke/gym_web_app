...
# Create
def save(workout):
    sql = "INSERT INTO workouts (name, category, date, start_time, capacity, booked) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id"
    values = [workout.name, workout.category, workout.date, workout.start_time, workout.capacity, workout.booked]
...
# Update
def update(workout):
    sql = "UPDATE workouts SET (name, category, date, start_time, capacity, booked) = (%s, %s, %s, %s, %s, %s) WHERE id = %s"
    values = [workout.name, workout.category, workout.date, workout.start_time, workout.capacity, workout.booked, workout.id]
    run_sql(sql, values)
